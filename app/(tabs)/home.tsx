...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, TextInput, Keyboard, KeyboardAvoidingView, Platform } from 'react-native';
...
// CRONÔMETRO INTELIGENTE: Ele mesmo expira a reserva
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (agora < reserva.inicioTimestamp) {
        ...
        onExpirar(); // Dispara o encerramento e o som!
      }
    };
    tick();
...
export default function HomeScreen() {
  ...
  const { recursos, reservarRecurso, notificacao, fecharNotificacao, finalizarReservaAutomatica } = useRecursos();
  ...
  const abrirModal = (item: Recurso) => {
    setRecursoSelecionado(item); setMatricula(''); setAviso(''); setHIni(''); setMIni(''); setHFim(''); setMFim(''); setModalVisivel(true);
  };
  ...
  const handleReservar = () => {
    ...
    if (fim <= inicio) { setTipoAviso('erro'); setAviso('O término deve ser superior ao início.'); return; } // Bloqueio de horário idêntico

    const duracaoH = (fim - inicio) / 3600000;
    if (duracaoH < recursoSelecionado.minHoras || duracaoH > recursoSelecionado.maxHoras) {
      setTipoAviso('erro'); setAviso(`Exigido ${formatarDuracaoVisual(recursoSelecionado.minHoras)} a ${formatarDuracaoVisual(recursoSelecionado.maxHoras)}.`); return;
    }

    try {
      reservarRecurso(recursoSelecionado.id, matricula, inicio, fim);
      setTipoAviso('sucesso'); setAviso('SUCESSO: Agendado.');
      setTimeout(() => { setModalVisivel(false); setAbaAtiva('AGENDADOS'); }, 1000);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {abaAtiva === 'CATALOGO' && (
          recursos.length === 0 ? <Text style={styles.textoVazio}>Nenhum recurso cadastrado.</Text> : recursos.map(item => (
            <TouchableOpacity key={item.id} style={styles.cardRecurso} onPress={() => abrirModal(item)}>
      ...
      {/* OVERLAY ABSOLUTO: O matador do bug de foco do teclado no Web */}
      {modalVisivel && (
...
const styles = StyleSheet.create({
  ...
  modalOverlayAbsoluto: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0, 0, 0, 0.9)', zIndex: 1000, justifyContent: 'flex-end' },
  ...
  modalContent: { backgroundColor: '#18181B', width: '100%', borderTopLeftRadius: 32, borderTopRightRadius: 32, padding: 32, borderWidth: 1, borderColor: '#27272A' },
