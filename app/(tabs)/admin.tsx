...
export default function AdminScreen() {
  ...
  const [nome, setNome] = useState(''); const [tipo, setTipo] = useState('SALA'); 
  ...
  const handleCadastrarRecurso = () => {
    ...
    if (!nome.trim()) { setTipoAviso('erro'); setAviso('O Nome é obrigatório.'); return; }
    if (recursos.some(r => r.nome.toLowerCase() === nome.toLowerCase())) { setTipoAviso('erro'); setAviso('Recurso já existe.'); return; }
    if (!minH || !minM || !maxH || !maxM) { setTipoAviso('erro'); setAviso('Preencha o relógio digital.'); return; }
    ...
    if (minDecimal > maxDecimal) { setTipoAviso('erro'); setAviso('Mínimo não pode ser maior que o máximo.'); return; }

    adicionarRecurso({ nome, tipo, minHoras: minDecimal, maxHoras: maxDecimal });
    setTipoAviso('sucesso'); setAviso(`SUCESSO: ${nome} cadastrado.`);
    setNome(''); setTipo('SALA'); setMinH(''); setMinM(''); setMaxH(''); setMaxM('');
  };
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity style={styles.btnGerenciar} onPress={() => router.push('/admin-lista')}><Text style={styles.txtGerenciar}>Acessar Recursos</Text></TouchableOpacity>

        <View style={styles.card}>
          <Text style={styles.sectionTitle}>Novo Recurso</Text>
          <Text style={styles.label}>Nome do Recurso</Text>
          <TextInput style={styles.input} placeholder="Ex: Lab Maker" placeholderTextColor="#52525B" value={nome} onChangeText={setNome} />
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.scrollChips} keyboardShouldPersistTaps="handled">
            {categorias.map(cat => (<TouchableOpacity key={cat} style={[styles.chip, tipo === cat && styles.chipAtivo]} onPress={() => setTipo(cat)}><Text style={[styles.chipTexto, tipo === cat && styles.chipTextoAtivo]}>{cat}</Text></TouchableOpacity>))}
          </ScrollView>
          ...
          <View style={styles.blocoTempo}>
            ...
            <View style={styles.relogioDigitalContainer}>
              <TextInput style={styles.inputRelogio} placeholder="00" placeholderTextColor="#52525B" keyboardType="numeric" maxLength={2} value={minH} onChangeText={(v) => processarTempo(v, 48, setMinH)} />
              <Text style={styles.separadorRelogio}>:</Text>
              <TextInput style={styles.inputRelogio} placeholder="00" placeholderTextColor="#52525B" keyboardType="numeric" maxLength={2} value={minM} onChangeText={(v) => processarTempo(v, 59, setMinM)} />
            </View>
          </View>
          ...
          <View style={styles.blocoTempo}>
            ...
            <View style={styles.relogioDigitalContainer}>
              <TextInput style={styles.inputRelogio} placeholder="00" placeholderTextColor="#52525B" keyboardType="numeric" maxLength={2} value={maxH} onChangeText={(v) => processarTempo(v, 48, setMaxH)} />
              <Text style={styles.separadorRelogio}>:</Text>
              <TextInput style={styles.inputRelogio} placeholder="00" placeholderTextColor="#52525B" keyboardType="numeric" maxLength={2} value={maxM} onChangeText={(v) => processarTempo(v, 59, setMaxM)} />
            </View>
          </View>
          ...
          <TouchableOpacity style={styles.btnConfirmar} onPress={handleCadastrarRecurso}><Text style={styles.txtConfirmar}>Registrar</Text></TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  saudacao: { fontSize: 24, fontWeight: 'bold', color: '#06B6D4', letterSpacing: -0.5 }, subSaudacao: { fontSize: 14, color: '#A1A1AA', marginTop: 4, textTransform: 'uppercase', letterSpacing: 1 },
  ...
  btnGerenciar: { backgroundColor: '#18181B', padding: 16, borderRadius: 12, borderWidth: 1, borderColor: '#06B6D4', alignItems: 'center', marginBottom: 24 }, txtGerenciar: { color: '#06B6D4', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1 },
  card: { backgroundColor: '#18181B', padding: 24, borderRadius: 24, borderWidth: 1, borderColor: '#27272A' },
  sectionTitle: { fontSize: 14, color: '#FAFAFA', fontWeight: '700', textTransform: 'uppercase', marginBottom: 24, letterSpacing: 1 }, label: { color: '#D4D4D8', marginBottom: 8, fontSize: 12, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  scrollChips: { marginBottom: 24, flexDirection: 'row' }, chip: { backgroundColor: '#09090B', paddingVertical: 10, paddingHorizontal: 16, borderRadius: 20, borderWidth: 1, borderColor: '#27272A', marginRight: 10, height: 45, justifyContent: 'center' },
  chipAtivo: { backgroundColor: '#131316', borderColor: '#06B6D4' }, chipTexto: { color: '#A1A1AA', fontSize: 12, fontWeight: 'bold', textTransform: 'uppercase' }, chipTextoAtivo: { color: '#06B6D4' },
  blocoTempo: { backgroundColor: '#09090B', padding: 16, borderRadius: 12, borderWidth: 1, borderColor: '#27272A', marginBottom: 20 }, labelTempo: { color: '#06B6D4', fontSize: 10, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 },
  btnConfirmar: { backgroundColor: '#06B6D4', padding: 18, borderRadius: 12, alignItems: 'center', marginTop: 8 }, txtConfirmar: { color: '#09090B', fontSize: 14, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1 },
